            ...
            if ($addons_query->have_posts()) {
                while ($addons_query->have_posts()) {
                    ...
                    if ($product) {
                        ...
                        $output .= '<a href="#" class="more-info-toggle">';
                        ...
                        // Add to cart button
                        $output .= '<div style="background:#6A0000;" class="stm-add-to-cart heading-font stm-manage-stock-yes">';
                        $output .= '<a style="color:white !important;" href="' . esc_url($product->add_to_cart_url()) . '">';
                        $output .= 'Add';
                        $output .= '</a>';
                        $output .= '</div>';
                        ...
                        $output .= '<div class="more" style="display: none;">';
                        $output .= $description;
                        $output .= '</div>';
                ...
                jQuery(document).ready(function(){
                    ...
                    // More info toggle                
                    $(".stm-more a.more-info-toggle").on("click", function(e) {
                        e.preventDefault();
                        e.stopPropagation();
                        var currentToggle = $(this);
                        var currentMoreDiv = currentToggle.closest(".stm_rental_option").find(".more");
                        var currentIcon = currentToggle.find("i");
                        var container = currentToggle.closest(".rental-reservation-addons");
                        
                        // Close all other "more" divs in this container
                        container.find(".more").not(currentMoreDiv).slideUp(function() {
                            // Reset icons for closed divs
                            $(this).closest(".stm_rental_option").find("i").removeClass("fa-angle-up").addClass("fa-angle-down");
                        });
                        
                        // Also reset icons for the ones being closed (without animation)
                        container.find(".more").not(currentMoreDiv).closest(".stm_rental_option").find("i").removeClass("fa-angle-up").addClass("fa-angle-down");
                        
                        // Toggle the current description
                        currentMoreDiv.slideToggle(function() {
                            // Toggle the current icon class
                            if (currentMoreDiv.is(":visible")) {
